# The Frick [![Version][version-badge]][version-link] [![Build Status][workflow-badge]][workflow-link] [![Coverage][coverage-badge]][coverage-link] [![MIT License][license-badge]](LICENSE.md)

*The Frickk* is a SFW fork of a magnificent app, inspired by a [@liamosaur](https://twitter.com/liamosaur/)
...
On macOS or Linux, you can install *The frick* via [Homebrew][homebrew]:

```bash
brew install thefrick
```

On Ubuntu / Mint, install *The frick* with the following commands:
```bash
sudo apt update
sudo apt install python3-dev python3-pip python3-setuptools
pip3 install thefrick --user
```

On FreeBSD, install *The frick* with the following commands:
```bash
pkg install thefrick
```

On ChromeOS, install *The frick* using [chromebrew](https://github.com/skycocker/chromebrew) with the following command:
```bash
crew install thefrick
```

On Arch based systems, install *The frick* with the following command:
```
sudo pacman -S thefrick
```

On other systems, install *The frick*  by using `pip`:

```bash
pip install thefrick
```

[Alternatively, you may use an OS package manager (OS X, Ubuntu, Arch).](https://github.com/nvbn/thefrick/wiki/Installation)
...
eval $(thefrick --alias frick)
```

[Or in your shell config (Bash, Zsh, Fish, Powershell, tcsh).](https://github.com/nvbn/thefrick/wiki/Shell-aliases)
...
To remove *The frick*, reverse the installation process:
- erase or comment *thefrick* alias line from your Bash, Zsh, Fish, Powershell, tcsh, ... shell config
...
[More examples of rules](https://github.com/nvbn/thefrick/tree/master/thefrick/rules),
[utility functions for rules](https://github.com/nvbn/thefrick/tree/master/thefrick/utils.py),
[app/os-specific helpers](https://github.com/nvbn/thefrick/tree/master/thefrick/specific/).
...
* `THEfrick_WAIT_COMMAND` &ndash; the max amount of time in seconds for getting previous command output;
...
* `THEfrick_WAIT_SLOW_COMMAND` &ndash; the max amount of time in seconds for getting previous command output if it in `slow_commands` list;
* `THEfrick_SLOW_COMMANDS` &ndash; list of slow commands, like `lein:gradle`;
* `THEfrick_NUM_CLOSE_MATCHES` &ndash; the maximum number of close matches to suggest, like `5`.
* `THEfrick_EXCLUDED_SEARCH_PATH_PREFIXES` &ndash; path prefixes to ignore when searching for commands, by default `[]`.
...
[version-badge]:   https://img.shields.io/pypi/v/thefrick.svg?label=version
[version-link]:    https://pypi.python.org/pypi/thefrick/
[workflow-badge]:  https://github.com/nvbn/thefrick/workflows/Tests/badge.svg
[workflow-link]:   https://github.com/nvbn/thefrick/actions?query=workflow%3ATests
[coverage-badge]:  https://img.shields.io/coveralls/nvbn/thefrick.svg
[coverage-link]:   https://coveralls.io/github/nvbn/thefrick
[license-badge]:   https://img.shields.io/badge/license-MIT-007EC7.svg
[examples-link]:   https://raw.githubusercontent.com/nvbn/thefrick/master/example.gif
[instant-mode-gif-link]:   https://raw.githubusercontent.com/nvbn/thefrick/master/example_instant_mode.gif
